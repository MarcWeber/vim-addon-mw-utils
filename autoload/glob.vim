...
fun! glob#Glob(pattern, ...)
  ...
  let c = getcwd()
  let cachable = get(opts, 'cachable', 0) && a:pattern[:len(c)-1] == c
  if cachable && has_key(s:glob_cache, a:pattern)
    return s:glob_cache[a:pattern]
  endif

  " FIXME: don't recurse into \.git directory (thus reimplement glob in vimL!)
  let r = filter(split(glob(a:pattern),"\n"),'v:val !~ '.string(s:c['regex_ignore_directory']))
  if cachable | let s:glob_cache[a:pattern]  = r | endif
  return r
endf
